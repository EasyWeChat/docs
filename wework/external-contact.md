...
$app->external_contact->getGroupChatList(int $statusFilter=0,array $useridList=[],array $partyidList=[],int $offset=0,int $limit=100);
```
...
$app->external_contact->getCorpTagList(array $tagIds=[]);
```
...
$app->external_contact->addCorpTag(string $groupId='',string $groupName,int $order=1,array $tags);
```
