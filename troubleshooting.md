...
2. 正确的支付授权目录应以 http:// 或 https:// 开头，并以正斜杠 / 结尾，授权目录所包含的域名必须经过 ICP 备案。

3. 支付授权目录需细化至二级或三级目录。

4. 所有实际调起微信支付请求的页面都必须要所配置的支付授权目录之下。

5. 在开发过程中，也可以使用测试授权目录进行开发测试，此时还应该将参与测试的个人微信号添加到测试白名单中，否则将出现对应的错误提示……  

> 配置前请先理解页面、目录、URL 以及域名等几个基本概念，并对自己所使用的框架的路由机制有一个大致了解。这样你才会知道自己正在配置的参数是个啥玩意儿，有什么卵用……:smile:
